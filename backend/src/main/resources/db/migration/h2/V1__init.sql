...
create table if not exists la_topic_channel_template (
  ...
  output_template clob,
  output_xml_template clob,
  transform_json clob,
  response_check_json clob,
  created_at timestamp,
  updated_at timestamp,
  unique(topic_id, channel_type)
);
...
create table if not exists la_api_key_scope (
  ...
  created_at timestamp,
  unique(api_key_id, scope_type, scope_id)
);
...
select 'u_admin', 'admin', '$2a$10$npZpvBMX1imudiwgxcK69epwpT8yhikyVIX4Y/unzzjztCQeZDfWe', true, current_timestamp

...
| 概念 | 说明 |
| --- | --- |
| User | 登录账号，包含用户名、密码哈希、启用状态 |
| Role | 权限集合，可由管理员维护；内置角色不可随意删除 |
| Permission | 细粒度权限字符串，如 `TOPIC_VIEW`、`APIKEY_CREATE`、`ROLE_UPDATE` |

用户不再拥有固定的基础角色（如历史 `ADMIN / USER`）。所有权限来自用户绑定的角色集合。

## 2. 初始管理员

启动时由配置引导创建初始管理员。初始 `admin` 用户会自动绑定内置的「超级管理员」角色（`r_super_admin`），从而获得全部权限。生产环境必须使用加密后的配置值和安全密钥：
...
| `GET /api/auth/me` | 根据 JWT 返回当前登录用户与有效权限列表 |

Token 使用 JWT HS256，密钥来自 `LITE_ALERT_JWT_SECRET`。前端通过 Axios 统一注入 `Authorization: Bearer <jwt>`。

## 5. 鉴权过滤器

- `JwtAuthFilter`：解析后台管理接口的 JWT，注入 `SecurityContext`。鉴权不再依赖 `ROLE_ADMIN`，而是通过 `PermissionService` 检查用户绑定的角色是否包含所需权限。
...
| `/api/namespaces/**` | `NAMESPACE_*` | 用户只能操作自己的命名空间；拥有 `NAMESPACE_VIEW_ALL` 可查看全部 |
| `/api/topics/**` | `TOPIC_*` | Topic CRUD、发布、禁用、模板 dry-run；拥有 `TOPIC_VIEW_ALL` 可查看全部 |
| `/api/apikeys/**` | `APIKEY_*` | ApiKey 列表、新建、编辑、撤销、轮换、删除 |
| `/api/contacts/**` | `CONTACT_*` | 通知目标管理；拥有 `CONTACT_VIEW_ALL` 可查看全部 |
| `/api/deliveries/**` | `DELIVERY_VIEW`，payload 需 `DELIVERY_PAYLOAD_READ` | 投递记录与敏感报文查看 |
| `/api/audit` | `AUDIT_VIEW` | 审计查询；拥有 `AUDIT_VIEW_ALL` 可查看全部日志 |
| `/api/admin/stats/**` | `STATS_VIEW` | 仪表盘全局统计 |
| `/api/admin/health` | `SYSTEM_HEALTH_VIEW` | 深度健康检查 |
| `/api/admin/mail-config` | `MAIL_CONFIG_VIEW` / `MAIL_CONFIG_UPDATE` / `SMTP_TEST` | SMTP 配置与测试 |
...
- `DELIVERY_PAYLOAD_READ` 单独拆分，避免普通用户默认看到原始业务报文。
